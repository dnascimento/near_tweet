...
public class ServerDispatcher extends Thread {
	private final Vector<TweetObject> mMessageList;
	...
	// sleep to avoid timestamp colisions
	public void sleep() {
		try {
			Thread.sleep(2);
	...
	public synchronized void dispatchMessage(ClientInfo aClientInfo,
		...
		case MSG:
			TweetServer.logInfo(aClientInfo.mSocket.getInetAddress()
					+ ":: TweetMsg");
			mMessageList.add(aMessage);
			// save image to disk for debug
		...
		case POLL_VOTE:
			TweetServer
					.logInfo("Client " + aClientInfo.mSocket.getInetAddress()
							+ ":: TweetPollVote");
			mMessageList.add(aMessage);
			break;
		...
		case GET:
			TweetServer.logInfo("Client "
					+ aClientInfo.mSocket.getInetAddress() + ":: TweetGet");
			break;
		...
		mMessageQueue.add(new Pair<ClientInfo, TweetObject>(aClientInfo,
				aMessage));
		notify();
	}
	...
	private synchronized Pair<ClientInfo, TweetObject> getNextMessageFromQueue()
		...
		Pair<ClientInfo, TweetObject> message = mMessageQueue.get(0);
